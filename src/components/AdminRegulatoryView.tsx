...
export const AdminRegulatoryView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="bg-tertiary-900 border border-tertiary-800 rounded-2xl p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-primary-300 bg-primary-950/80 px-2.5 py-1 rounded-full border border-primary-800/60">
      ...
      <div className="bg-tertiary-900 border border-tertiary-800 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="overflow-x-auto rounded-xl border border-tertiary-800">
          <table className="w-full text-left text-xs">
            <thead className="bg-tertiary-950 text-slate-400 font-semibold border-b border-tertiary-800 uppercase tracking-wider text-[11px]">
              <tr>
            ...
            <tbody className="divide-y divide-tertiary-800 text-slate-300">
              {(courses || []).map((c) => {
                const isEditing = editingCourseId === c.id;

                if (isEditing) {
                  return (
                    <tr key={c.id} className="bg-tertiary-800/50">
                      <td className="p-3 space-y-1">
                        <input
                          type="text"
                          value={editForm.title || ''}
                          onChange={(e) => setEditForm({ ...editForm, title: e.target.value })}
                          className="w-full bg-tertiary-950 border border-tertiary-700 rounded px-2 py-1 text-xs text-white"
                        />
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-16 bg-tertiary-950 border border-tertiary-700 rounded px-2 py-1 text-xs text-white"
                          />
                      ...
                      <td className="p-3">
                        <input
                          ...
                          className="bg-tertiary-950 border border-tertiary-700 rounded px-2 py-1 text-xs text-white"
                        />
                      </td>
                      <td className="p-3">
                        <input
                      ...
                      <td className="p-3 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 bg-tertiary-700 hover:bg-tertiary-600 text-slate-300 rounded transition-colors"
                          >
                ...
                return (
                  <tr key={c.id} className="hover:bg-tertiary-800/30 transition-colors">
                    <td className="p-3">
                    ...
                    <td className="p-3 capitalize">{c.sector.replace('_', ' ')}</td>
                    <td className="p-3">
                      <span className="px-2 py-0.5 rounded bg-tertiary-800 text-primary-300 font-semibold text-[11px]">
                    ...
                    <td className="p-3">
                      {c.sourceUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-primary-400 hover:underline text-[11px]"
                        >
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 bg-tertiary-800 hover:bg-tertiary-700 text-slate-300 hover:text-white rounded border border-tertiary-700 text-[11px] font-semibold transition-colors"
                      >
      ...
      <div className="bg-tertiary-900 border border-tertiary-800 rounded-2xl p-6 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-white flex items-center gap-2">
        ...
        {(changelog || []).length === 0 ? (
          <div className="p-4 bg-tertiary-950 rounded-xl border border-tertiary-800 text-center text-xs text-slate-500">
          ...
          <div className="space-y-2">
            {(changelog || []).map((entry) => (
              <div
                key={entry.id}
                className="p-3 bg-tertiary-950 border border-tertiary-800 rounded-xl flex items-start justify-between gap-4 text-xs"
              >
                <div className="space-y-1">
                  ...
                  <p className="text-slate-300">
                    Modified <span className="font-semibold text-primary-400">{entry.entityId}</span> ({entry.changedField}): {entry.newValue}
                  </p>
                  <span className="text-[11px] text-slate-500">Citation: {entry.sourceReference}</span>
                </div>
                <span className="px-2 py-0.5 rounded bg-primary-950 text-primary-400 text-[10px] font-bold shrink-0">
