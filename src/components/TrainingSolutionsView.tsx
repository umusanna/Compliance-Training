      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto justify-end">
          <div className="flex items-center space-x-1 bg-slate-950 border border-slate-800 rounded-xl px-2.5 py-1">
            ...
            >
              <option value="all" className="bg-slate-900">All Sectors & Universal</option>
              <option value="food_hospitality" className="bg-slate-900">Food & Hospitality (FSA)</option>
              <option value="health_social_care" className="bg-slate-900">Health & Social Care (CQC)</option>
              <option value="financial_services" className="bg-slate-900">Financial Services (FCA)</option>
              <option value="construction" className="bg-slate-900">Construction (HSE CDM)</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredCourses.map((course) => {
          ...
          return (
            <div
              key={course.id}
              className={`bg-slate-900 border rounded-2xl p-5 shadow-sm flex flex-col justify-between transition-shadow hover:border-slate-700 ${
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {/* Duration pill */}
                  <div className="flex items-center space-x-1 text-slate-400 text-xs shrink-0 bg-slate-950 border border-slate-800 px-2.5 py-1 rounded-lg">
                ...
                {course.plainLanguageHelp && (
                  <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 flex items-start gap-2">
                ...
                {/* Statutory Driver */}
                <div className="bg-slate-950 rounded-xl p-3 text-xs border border-slate-800 space-y-1">
                ...
                <div className="flex items-center space-x-1.5 text-xs text-slate-400">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {course.targetRoles.map((r) => (
                      <span
                        key={r}
                        className="bg-slate-800 text-slate-300 font-medium text-[10px] px-2 py-0.5 rounded capitalize"
                      >
                        {r.replace('_', ' ')}
                      </span>
                    ))}
              ...
              <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
                ...
                {hasGapInCurrentBusiness && !readOnly && (
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs px-3.5 py-1.5 rounded-lg transition-colors shadow-sm shrink-0"
                  >
